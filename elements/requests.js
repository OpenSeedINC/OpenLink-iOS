function send_request(requestid, message) {
    ...
    var url = "http://142.93.27.131:8675/areas.php"
    var d = new Date()
    ...
    http.send("devid=" + devId + "&appid=" + appId + "&id=" + userid
              + "&roomid=" + room + "&requestid=" + requestid
              + "&name=" + usercardNum + "&message=" + message + "&type=send")
}
...
function check_requests(where) {
    ...
    http.onreadystatechange = function () {
        if (http.readyState === 4) {
            if (http.responseText === 100) {
                ...
                requests = 0
                for (var num = 1; num < server.split(
                         "><").length; num = num + 1) {
    ...
    http.open('POST', url.trim(), true)
    http.setRequestHeader("Content-type", "application/x-www-form-urlencoded")
    http.send("devid=" + devId + "&appid=" + appId + "&id=" + userid + "&name="
              + usercardNum + "&type=check")
}
...
function accept_request(room) {
    ...
    http.setRequestHeader("Content-type", "application/x-www-form-urlencoded")
    http.send("devid=" + devId + "&appid=" + appId + "&id=" + userid
              + "&roomid=" + room + "&type=update&status=1")
}
...
function decline_request(room) {
    ...
    http.setRequestHeader("Content-type", "application/x-www-form-urlencoded")
    http.send("devid=" + devId + "&appid=" + appId + "&id=" + userid
              + "&roomid=" + room + "&type=update&status=2")
}
