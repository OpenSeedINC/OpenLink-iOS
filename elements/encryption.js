...
function genkey(extra, type, forwhom) {
    ...
    db.transaction(function (tx) {
        ...
        tx.executeSql(
                    "CREATE TABLE IF NOT EXISTS keys (ekey TEXT,type TEXT,forwhom TEXT)")

        var pull = tx.executeSql(
                    "SELECT * FROM keys WHERE forwhom ='" + forwhom + "'")
        ...
        if (pull.rows.length === 0) {
            ...
            for (num = 0; num < 40000; num = num + 1) {
                ...
                var splitby1 = end.charAt(
                            Math.floor((Math.random() * (end.length - 5)) + 1))
...
function encrypt(key, message) {
    ...
    message = "</message/>" + userid.toUpperCase(
                ) + "</message/>" + message + "</message/>" + userid.toUpperCase(
                ) + "</message/>"
    var salt = ""
    var encrypted1 = ""
    var encrypted2 = ""
    ...
    /* in future versions we'll need to do this a couple times, to make sure the mix is good and mixed. For now once will do. */
    for (var num = 0; num < salt.length; num = num + 1) {
        encrypted1 = encrypted1 + message.charAt(num) + salt.slice(num,
                                                                   encryptnum)
    }
...
function decrypt(key, message) {
    ...
    for (var num = 0; num <= message.length; num = num + alphaoffset) {
        ...
        if (alpha10.indexOf(message.slice(num, num + 2)) !== -1) {
            decrypted3 = decrypted3 + alpha10.indexOf(message.slice(num,
                                                                    num + 2))
            alphaoffset = 2
        } else if (Number.isNaN(Number(message.slice(num, num + 3))) === false
                   && message.slice(num, num + 3) >= 100 && message.slice(
                       num, num + 3) <= 126) {

            decrypted3 = decrypted3 + String.fromCharCode(message.slice(
                                                              num, num + 3))
            alphaoffset = 3
        } else if (Number.isNaN(Number(message.slice(num, num + 2))) === false
                   && message.slice(num, num + 2) >= 32) {
